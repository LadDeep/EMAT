...
const GroupActivitiesList = ({ groupId, noOfParticipants, userId, activities }) => {
  const expenses = activities.map((expense) => {
    const lent_or_borrowed_amount = noOfParticipants===1 ? expense.amount:
      expense.amount - expense.amount / noOfParticipants;
    const user_name = expense.id===userId ? "You" : expense.name;
